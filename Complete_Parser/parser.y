...
%type<node> program block stmts stmt var_dec fun_dec proc_dec fun_dec_args iden const arith bool expr call_args type print do_loop while_loop var_assign
...
var_assign: iden ASSIGN expr { tree = insert($1->name, strdup("var_assign"), currentNode++, $1, $3); $$ = tree; }
|   var_dec ASSIGN expr { tree = insert(strdup("NULL"), strdup("var_assign"), currentNode++, $1, $3); $$ = tree; }
;
...
print: K_PRINT_INTEGER LPAREN iden RPAREN { tree = insert($3->name, strdup("Print"), currentNode++, $3, NULL); $$ = tree; }
|   K_PRINT_INTEGER LPAREN const RPAREN { tree = insert($3->name, strdup("Print"), currentNode++, $3, NULL); $$ = tree; }
|   K_PRINT_DOUBLE LPAREN iden RPAREN { tree = insert($3->name, strdup("Print"), currentNode++, $3, NULL); $$ = tree; }
|   K_PRINT_DOUBLE LPAREN const RPAREN { tree = insert($3->name, strdup("Print"), currentNode++, $3, NULL); $$ = tree; }
|   K_PRINT_STRING LPAREN iden RPAREN { tree = insert($3->name, strdup("Print"), currentNode++, $3, NULL); $$ = tree; }
...
|   K_DO LPAREN bool RPAREN stmt { tree = insert(strdup("NULL"), strdup("DoLoop"), currentNode++, $3, $5); $$ = tree; }
;

while_loop: K_WHILE LPAREN bool RPAREN block { tree = insert(strdup("NULL"), strdup("WhileLoop"), currentNode++, $3, $5); $$ = tree; }
|   K_WHILE LPAREN bool RPAREN stmt { tree = insert(strdup("NULL"), strdup("WhileLoop"), currentNode++, $3, $5); $$ = tree; }
;
