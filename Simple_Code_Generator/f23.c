 ...
 *
 *      F23_Exit        Prints final statistics and exits
...
#define F23_MemSize     100000
#define F23_RSize       32
#define F23_FSize       16
#define F23_SbufSize    1024

double  F[F23_FSize];           /* Floaing point registers          */
int     R[F23_RSize];           /* Integer registers                */
int     Mem[F23_MemSize];       /* Memory                           */
int     F23_Time;               /* Timer                            */
char    F23_SBuf[F23_SbufSize]; /* String input buffer              */
...
void F23_Exit( int rc ) {
    ...
    printf( "\n\nF23_Exit called. Statistics follow:\n");
...
int get_string() {
     ...
     * in the global variable F23_SBuf
     *
     ****************************************************************/
...
enum {  F23_Calc_Abs,           /* abs(d1)                          */
        F23_Calc_Pow,           /* pow(d1,d2)                       */
        F23_Calc_Sqrt           /* sqrt(d1)                         */
     };
...
int allocate_in_Mem( int nints ) {
    ...
    if ( Mem[hdr+1] >= SR ) {
        printf( "F23 Runtime Error: allocate_in_Mem heap overflow\n" );
        F23_Exit( 3 );
    }
...
void free_in_Mem( int ptr ) {
    ...
    if ( ptr < Allocate_Block_Header ||
         ptr+Allocate_Block_Header >= F23_MemSize ) {
        printf( "F23 Runtime Error: free_in_Mem ptr out of bounds\n" );
        F23_Exit( 1 );
    }

    hdr = ptr - 2;
    if ( Mem[hdr] != Allocate_Block_InUse ) {
        printf( "F23 Runtime Error: free_in_Mem ptr to unallocated block\n" );
        F23_Exit( 2 );
    }
...
int main(int argc, char **argv) {
    ...
    for( i = 0; i < F23_RSize; i++ ) R[i] = 0;
    for( i = 0; i < F23_FSize; i++ ) F[i] = 0.0;
    for( i = 0; i < F23_MemSize; i++ ) Mem[i] = 0;
